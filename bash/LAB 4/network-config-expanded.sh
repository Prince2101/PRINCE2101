#!/bin/bash
#
# this script displays some host identification information for a Linux machine
#
# Sample output:
#   Hostname      : zubu
#   LAN Address   : 192.168.2.2
#   LAN Name      : net2-linux
#   External IP   : 1.2.3.4
#   External Name : some.name.from.our.isp

# the LAN info in this script uses a hardcoded interface name of "eno1"
#    - change eno1 to whatever interface you have and want to gather info about in order to test the script

# TASK 1: Dynamically identify the list of interface names for the computer running the script, and use a for loop to generate the report for every interface except loopback

################
# Data Gathering
################
# the first part is run once to get information about the host
# grep is used to filter ip command output so we don't have extra junk in our output
# stream editing with sed and awk are used to extract only the data we want displayed

#####
# Once per host report
#####
# we use the hostname command to get our system name
my_hostname=$(hostname)

# the default route can be found in the route table normally
# the router name is obtained with getent
default_router_address=$(ip r s default| cut -d ' ' -f 3)
default_router_name=$(getent hosts $default_router_address|awk '{print $2}')

# finding external information relies on curl being installed and relies on live internet connection
external_address=$(curl -s icanhazip.com)
external_name=$(getent hosts $external_address | awk '{print $2}')


cat <<EOF
System Identification Summary
=============================
Hostname      : $my_hostname
Default Router: $default_router_address
Router Name   : $default_router_name
External IP   : $external_address
External Name : $external_name
EOF

#####
# End of Once per host report
#####

# the second part of the output generates a per-interface report
# the task is to change this from something that runs once using a fixed value for the interface name to
#   a dynamic list obtained by parsing the interface names out of a network info command like "ip"
#   and using a loop to run this info gathering section for every interface found

# the default version uses a fixed name and puts it in a variable
#####
# Per-interface report
#####
# define the interface being summarized
#interface="eth0"

# Find an address and hostname for the interface being summarized
# we are assuming there is only one IPV4 address assigned to this interface

loopback="lo" #setting variable to check if interface is loopback
interface_list=() # creating empty array/list to hold data of active interfaces

for intface in $(ifconfig | cut -d ' ' -f1| tr ':' '\n' | awk NF) # using for loop to itterate through active interfaces
do

        interface_list+=("$intface") # adding value of intface i.e. interfaces on interface_list array
done # exiting for loop

for interface in ${interface_list[@]}; do #using for loop on interface_list to itterate through data stored their
ipv4_address=$(ip a s $interface|awk -F '[/ ]+' '/inet /{print $3}') # the data of first index of list is stored initially and as loop continues data changes to second, third simultaneously
ipv4_hostname=$(getent hosts $ipv4_address | awk '{print $2}') # initially hostname of first index is stored which simultaneously changes with continuation of for loop

# Identify the network number for this interface and its name if it has one
network_address=$(ip route list dev $interface scope link|cut -d ' ' -f 1) # calculating and storing network address of interface on first index and continuation with loop
network_number=$(cut -d / -f 1 <<<"$network_address")
network_name=$(getent networks $network_number|awk '{print $1}')
if echo "$interface" | grep -q "$loopback"; then # using if statement to verify if interface name begins with lo. only loopback interface name begins with lo i.e loopback
echo "" # breaking line
echo "Loop-back interface data not recorded" # since no data need to be shown regarding loopback interfaces we notify user about that
echo "" #breaking line
else
cat <<EOF   # using cat command to show required data.
Interface $interface:
===============
Address         : $ipv4_address
Name            : $ipv4_hostname
Network Address : $network_address
Network Name    : $network_name
Network Number : $network_number
EOF
fi # exiting if statement
done # exiting for loop


#####
# End of per-interface report
#####
